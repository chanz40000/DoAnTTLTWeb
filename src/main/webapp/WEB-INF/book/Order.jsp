...
<%-- Lấy userId từ session và chuyển đổi sang kiểu int --%>
<c:set var="userIdString" value="${sessionScope.userC.getUserId()}"/>
<p>${userIdString}</p>
<%-- Chuyển userIdString sang kiểu int bằng parseInt() --%>
<%-- Nếu không thể chuyển đổi, mặc định userId thành 0 --%>
<%--<c:set var="userId" value="${Integer.parseInt(userIdString)}" />--%>
<%-- Chuyển đổi statusId sang kiểu int --%>
<c:set var="statusId" value="1" />

<%-- Sử dụng phương thức selectByUserId(userId) của orderDAO --%>
<%--<c:set var="orders" value="${orderDAO.selectByUserId(userIdString)}"/>--%>
...
<div class="order">
    <table id="example" class="table table-striped" style="width:100%">
        ...
        <tbody>
        <c:set var="userIdInt" value="${Integer.parseInt(userIdString)}" />


        <c:forEach var="order" items="${orderDAO.selectByUserId(userIdInt)}">
            <tr>
